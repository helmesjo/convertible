...
namespace convertible::converter
{
  ...
  {
    constexpr auto
    operator()(auto&& val) const -> decltype(auto)
    {
      return FWD(val);
    }
  };
  ...
  struct explicit_cast
  {
    ...
    operator()(auto&& obj) const -> decltype(auto)
      requires std::common_reference_with<std::remove_cvref_t<target_t>,
                                          converted_t<decltype(obj)>> ||
               concepts::castable_to<converted_t<decltype(obj)>, target_t>
    {
      if constexpr (std::common_reference_with<std::remove_cvref_t<target_t>,
                                               converted_t<decltype(obj)>>)
      {
