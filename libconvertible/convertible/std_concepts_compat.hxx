...
namespace std
{
  ...
  concept assignable_from = std::is_lvalue_reference_v<lhs_t> && requires (lhs_t lhs, rhs_t&& rhs) {
                                                                   {
  ...
  concept convertible_to =
    std::is_convertible_v<from_t, to_t> && requires { static_cast<to_t>(std::declval<from_t>()); };
...
namespace std
{
  ...
  forward_like(U&& x) noexcept -> auto&&
  {
