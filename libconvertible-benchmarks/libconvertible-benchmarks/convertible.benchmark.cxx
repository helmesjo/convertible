...
namespace
{
  ...
  auto
  gen_random_str(std::size_t len)
  {
    static char const alphanum[] = "0123456789"
    ...
    {
      tmpS += alphanum[gen_random_int() % (sizeof(alphanum) - 1)];
    }
  ...
  {
    auto
    operator()(std::string const& s) const -> int
    {
      return std::stoi(s);
    }

    auto
    operator()(int i) const -> std::string
    {
  ...
  create_type_a()
  {
    ...
    type_a obj{gen_random_int(), gen_random_str(size), {}, 999};
  ...
  create_type_b()
  {
    ...
    type_b obj{gen_random_int(), gen_random_str(size), {}, 0};
...
{
  auto table =
    mapping_table{mapping(member(&type_a::val1), member(&type_b::val1)),
